...
def test_page_request_validation():
    ...
    with pytest.raises(ValidationError):
        PageRequest(number=0, size=0)


def test_page_request_of_size():
    page_request = PageRequest.of_size(10)
    assert page_request.size == 10
    assert page_request.number == 0


def test_page_request_offset():
    page_request = PageRequest(number=3, size=10)
    assert page_request.offset() == 30


def test_page_request_first():
    page_request = PageRequest(number=3, size=10).first()
    assert page_request.number == 0
    assert page_request.size == 10


def test_page_request_next():
    page_request = PageRequest(number=3, size=10).next()
    assert page_request.number == 4
    assert page_request.size == 10


def test_page_request_previous():
    page_request = PageRequest(number=3, size=10).previous()
    assert page_request.number == 2
    assert page_request.size == 10

    page_request = PageRequest(number=0, size=10).previous()
    assert page_request.number == 0
    assert page_request.size == 10
...
def test_page_request_has_previous():
    ...
    page_request = PageRequest(number=0, size=10)
    assert not page_request.has_previous()


def test_page_number():
    page = Page(content=[1, 2, 3], page_request=PageRequest(number=1, size=10), total_elements=13)
...
def test_page_has_next():
    page = Page(content=[1, 2, 3, 4, 5], page_request=PageRequest(number=1, size=5), total_elements=25)
    assert page.has_next()

    page = Page(content=[1, 2, 3, 4, 5], page_request=PageRequest(number=5, size=5), total_elements=25)
...
def test_page_has_previous():
    ...
    page = Page(content=[1, 2, 3, 4, 5], page_request=PageRequest(number=0, size=5), total_elements=25)
...
def test_page_next_page_request():
    page = Page(content=[1, 2, 3, 4, 5], page_request=PageRequest(number=1, size=5), total_elements=25)
    assert page.next_page_request().number == 2
...
def test_page_previous_page_request():
    page = Page(content=[1, 2, 3, 4, 5], page_request=PageRequest(number=1, size=5), total_elements=25)
    assert page.previous_page_request().number == 0
